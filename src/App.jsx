...
function PlanItGrid({ item, plan, onPlan, onClose }) {
  ...
  const [selected, setSelected] = React.useState([]); // [{ day, slot }, ...]

  const toggle = (day, slot) => {
    const key = `${day}|${slot}`;
    setSelected(prev =>
      prev.find(s => s.day === day && s.slot === slot)
        ? prev.filter(s => !(s.day === day && s.slot === slot))
        : [...prev, { day, slot }]
    );
  };

  const confirm = () => {
    onPlan(item, selected);
    onClose();
  };

  return (
    <div className="plan-grid">
      <div className="plan-grid-title">📅 Pick slots — then confirm</div>
          ...
          {validSlots.map(slot => (
            <tr key={slot}>
              ...
              {DAYS.map(day => {
                ...
                const picked = selected.find(s => s.day === day && s.slot === slot);
                return (
                  <td key={day}>
                    <button
                      className={`plan-slot-btn ${taken ? "taken" : picked ? "picked" : "empty"}`}
                      disabled={taken}
                      onClick={() => !taken && toggle(day, slot)}
                      title={taken ? `${day} ${slot} taken` : `${day} ${slot}`}
                    >
                      {taken ? "✓" : picked ? "●" : "+"}
                    </button>
                  </td>
      ...
      <div style={{ display: "flex", gap: "6px", marginTop: "8px" }}>
        {selected.length > 0 && (
          <button className="plan-grid-confirm" onClick={confirm}>
            ✓ Add to {selected.length} slot{selected.length > 1 ? "s" : ""}
          </button>
        )}
        <button className="plan-grid-cancel" onClick={onClose}>cancel</button>
      </div>
    </div>
  );
}
...
export default function MealPlanner() {
  ...
  const [mobileThisWeekOpen, setMobileThisWeekOpen] = useState(false);
  const [mobileScratchPickerOpen, setMobileScratchPickerOpen] = useState(false);
  const [bankPlanningId, setBankPlanningId] = useState(null); // { meal, day, slot, type, portions }
  ...
  const planFromScratchpad = (meal, slots) => {
  ...
  const planIntoWeekMulti = (item, slots) => {
    slots.forEach(({ day, slot }) => {
      setPlan(prev => ({ ...prev, [day]: { ...prev[day], [slot]: { ...item, isInventory: true, invType: item.type } } }));
    });
    setInventory(prev => prev.map(i => i.id === item.id ? { ...i, portions: Math.max(0, i.portions - 1) } : i)
  ...
  const planIntoWeek = (item, slots) => {
    ...
    setInventory(prev => prev.map(i => i.id === item.id ? { ...i, portions: Math.max(0, i.portions - 1) } : i).filter(i => i.portions > 0));
    setPlanningItem(null);
  };
  ...
  const filteredMeals = meals.filter(m => {
    ...
    const matchSlot = !filterSlot || (m.mealTypes?.length && m.mealTypes.includes(filterSlot));
    const matchSelecting = !selecting || !m.mealTypes?.length || m.mealTypes.includes(selecting.slot);
    return matchSearch && matchTag && matchSlot && matchSelecting;
  });
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .meal-card-actions { display: flex; gap: 6px; margin-top: 10px; }
        .meal-card-action { flex: 1; padding: 6px; border-radius: 7px; font-family: 'Nunito', sans-serif; font-size: 11px; font-weight: 700; cursor: pointer; transition: all 0.15s; text-align: center; }
        .meal-card-action.queue { background: #faf7f2; border: 1.5px solid #e0d4c4; color: #8a7060; }
        .meal-card-action.queue:hover { border-color: #e8a045; color: #e8a045; background: #fffbf4; }
        .meal-card-action.queue.queued { background: #dcfce7; border-color: #4a7c59; color: #4a7c59; cursor: default; }
        .meal-card-action.slot { background: #2d2416; border: 1.5px solid #2d2416; color: #f5e6c8; }
        .meal-card-action.slot:hover { background: #e8a045; border-color: #e8a045; color: #fff; }
        .meal-card-action.slot.active { background: #e8a045; border-color: #e8a045; color: #fff; }
        ...
        .mobile-day-card { background: #fff; border: 1.5px solid #ede5d8; border-radius: 14px; padding: 12px 14px; margin-bottom: 10px; }
        .mobile-day-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
        .mobile-day-name { font-family: 'Lora', serif; font-size: 16px; font-weight: 600; color: #2d2416; }
        .mobile-day-count { font-size: 11px; color: #a89070; background: #f5ede0; padding: 2px 8px; border-radius: 999px; }
        .mobile-slots-row { display: flex; flex-direction: column; gap: 6px; }
        .mobile-slot-pill { display: flex; align-items: center; gap: 10px; padding: 9px 12px; border-radius: 10px; border: 1.5px solid #ede5d8; background: #faf7f2; cursor: pointer; transition: border-color 0.15s; }
        .mobile-slot-pill.filled { background: #fffbf4; border-color: #e8c07a; }
        .mobile-slot-pill.empty:active { background: #f5ede0; border-color: #e8a045; }
        .mobile-slot-icon { font-size: 16px; flex-shrink: 0; }
        .mobile-slot-content { flex: 1; display: flex; align-items: center; justify-content: space-between; min-width: 0; }
        .mobile-slot-meal { font-size: 13px; font-weight: 700; color: #2d2416; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; }
        .mobile-slot-remove { background: none; border: none; color: #c4b4a4; font-size: 18px; cursor: pointer; padding: 0 0 0 8px; line-height: 1; flex-shrink: 0; }
        .mobile-slot-empty { font-size: 13px; color: #c4b4a4; font-weight: 600; }
        .mobile-this-week { background: #fff; border: 1.5px solid #ede5d8; border-radius: 14px; overflow: hidden; margin-top: 4px; }
        .mobile-this-week-header { background: #2d2416; padding: 12px 14px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; }
        ...
        .plan-slot-btn.picked { background: #e8a045; color: #fff; border-color: #d4903a; }
        .plan-grid-confirm { flex: 1; padding: 7px 12px; background: #e8a045; color: #fff; border: none; border-radius: 7px; font-family: 'Nunito', sans-serif; font-size: 12px; font-weight: 700; cursor: pointer; }
        .plan-grid-confirm:hover { background: #d4903a; }
        .plan-grid-cancel { padding: 6px 12px; background: transparent; border: 1px solid #e0d4c4; border-radius: 7px; color: #a89070; font-family: 'Nunito', sans-serif; font-size: 11px; cursor: pointer; }
          ...
          {view === "planner" && (
            ...
            >
                      ...
                      {scratchPlanningId === meal.id && (
                        <div style={{ marginTop: "8px" }}>
                          <PlanItGrid
                            ...
                            plan={plan}
                            onPlan={(item, slots) => planFromScratchpad(meal, slots)}
              ...
              <div className="mobile-day-view">
                {DAYS.map(day => {
                  const filledSlots = SLOTS.filter(s => plan[day][s]);
                  return (
                    <div key={day} className="mobile-day-card">
                      <div className="mobile-day-header">
                        <span className="mobile-day-name">{day}</span>
                        <span className="mobile-day-count">{filledSlots.length}/{SLOTS.length} meals</span>
                      </div>
                      <div className="mobile-slots-row">
                        {SLOTS.map(slot => {
                          const meal = plan[day][slot];
                          return (
                            <div key={slot} className={`mobile-slot-pill ${meal ? "filled" : "empty"}`}
                              onClick={() => !meal && setSelecting({ day, slot })}>
                              <span className="mobile-slot-icon">{SLOT_ICONS[slot]}</span>
                              {meal ? (
                                <div className="mobile-slot-content">
                                  <div className="mobile-slot-meal">{meal.name}</div>
                                  <button className="mobile-slot-remove" onClick={e => { e.stopPropagation(); removeMeal(day, slot); }}>×</button>
                                </div>
                              ) : (
                                <span className="mobile-slot-empty">+ Add</span>
                              )}
                            </div>
                          );
                        })}
                ...
                <div className="mobile-this-week">
                  <div className="mobile-this-week-header" onClick={() => setMobileThisWeekOpen(p => !p)}>
                    <div className="scratch-title">📝 This Week {scratchpad.length > 0 && <span className="scratch-count">{scratchpad.length}</span>}</div>
                    <span className="mobile-this-week-toggle">{mobileThisWeekOpen ? "▲" : "▼"}</span>
                  </div>
                  {mobileThisWeekOpen && (
                    <div className="scratch-body">
                      {scratchpad.length === 0 && !mobileScratchPickerOpen && (
                      ...
                      {scratchpad.map(meal => (
                        <div key={meal.id} className="scratch-item">
                          ...
                          <div className="scratch-item-btns">
                            <button className="scratch-plan-btn" onClick={() => setScratchPlanningId(scratchPlanningId === meal.id ? null : meal.id)}>
                              {scratchPlanningId === meal.id ? "Cancel" : "📅 Slot it"}
                            </button>
                            <button className="scratch-remove-btn" onClick={() => removeFromScratchpad(meal.id)}>×</button>
                          </div>
                        ...
                        <div className="scratch-picker">
                          {scratchNewMeal ? (
                            <div className="scratch-new-form">
                              ...
                              <span className="scratch-new-label">Ingredients (optional)</span>
                              <input
                                placeholder="chicken, potatoes..."
                                value={scratchNewMeal.ingredients || ""}
                                onChange={e => setScratchNewMeal(p => ({ ...p, ingredients: e.target.value }))}
                              />
                              <button className="scratch-new-save" onClick={saveScratchNewMeal}>+ Add to bank & queue</button>
                            ...
                            <>
                              ...
                              <div className="scratch-picker-list">
                                {meals
                                  ...
                                  .map(m => {
                                    const already = scratchpad.find(s => s.id === m.id);
                                    return (
                                      <div key={m.id} className={`scratch-picker-item ${already ? "already" : ""}`}
                                        onClick={() => !already && addToScratchpad(m)}>
                                        <span>{m.name}</span>
                                        {already ? <span style={{ fontSize: "10px" }}>✓ added</span> : <span style={{ color: "#e8a045", fontSize: "11px" }}>+</span>}
                                      </div>
                              ...
                              <button style={{ width: "100%", padding: "6px", marginTop: "4px", background: "transparent", border: "none", color: "#a89070", fontSize: "11px", cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}
                                onClick={() => { setMobileScratchPickerOpen(false); setScratchSearch(""); setScratchNewMeal(null); }}>close</button>
                            </>
                          )}
                        </div>
                      ) : (
                        <button className="scratch-add-btn" onClick={() => setMobileScratchPickerOpen(true)}>+ Add a meal idea</button>
                      )}
                    </div>
                  )}
                </div>
          ...
          {view === "meals" && (
            <>
                ...
                {SLOTS.map(s => (
                  <button key={s}
                    ...
                    style={{
                      ...
                      borderColor: filterSlot === s ? "#2d2416" : "#e0d4c4",
                      background: filterSlot === s ? "#2d2416" : "#fff",
                      color: filterSlot === s ? "#f5e6c8" : "#8a7060",
                      fontFamily: "'Nunito', sans-serif", fontSize: "12px", fontWeight: 700, cursor: "pointer"
                    }}>{SLOT_ICONS[s]} {s}</button>
                ))}
              </div>
                ...
                {TAGS.map(t => (
                  <button key={t}
                    ...
                    style={{
                      ...
                      borderColor: filterTag === t ? TAG_COLORS[t]?.text || "#e8a045" : "#e0d4c4",
                      background: filterTag === t ? (TAG_COLORS[t]?.bg || "#fef3c7") : "#fff",
                      color: filterTag === t ? (TAG_COLORS[t]?.text || "#92400e") : "#8a7060",
                      fontFamily: "'Nunito', sans-serif", fontSize: "11px", fontWeight: 700, cursor: "pointer"
                    }}>{t}</button>
                ))}
                {(filterTag || filterSlot || search) && (
                  <button onClick={() => { setFilterTag(null); setFilterSlot(null); setSearch(""); }}
                    style={{ padding: "4px 10px", borderRadius: "999px", border: "1.5px solid #fca5a5", background: "#fff1f1", color: "#f87171", fontFamily: "'Nunito', sans-serif", fontSize: "11px", fontWeight: 700, cursor: "pointer" }}>
                    ✕ Clear
                  </button>
                )}
              </div>
              ...
              {filteredMeals.length === 0 && (
                <div style={{ textAlign: "center", padding: "40px 20px", color: "#a89070", fontSize: "14px" }}>
              ...
              <div className="meals-grid">
                {filteredMeals.map(m => {
                  const inQueue = !!scratchpad.find(s => s.id === m.id);
                  const isSlotting = bankPlanningId === m.id;
                  return (
                  <div key={m.id} className="meal-card">
                    ...
                    <div className="meal-card-actions">
                      <button
                        className={`meal-card-action queue ${inQueue ? "queued" : ""}`}
                        onClick={() => !inQueue && addToScratchpad(m)}
                      >{inQueue ? "✓ In This Week" : "📝 This Week"}</button>
                      <button
                        className={`meal-card-action slot ${isSlotting ? "active" : ""}`}
                        onClick={() => setBankPlanningId(isSlotting ? null : m.id)}
                      >{isSlotting ? "Cancel" : "📅 Slot it"}</button>
                    </div>
                    {isSlotting && (
                      <div style={{ marginTop: "8px" }}>
                        <PlanItGrid
                          ...
                          onPlan={(item, slots) => {
                            slots.forEach(({ day, slot }) => {
                              setPlan(prev => ({ ...prev, [day]: { ...prev[day], [slot]: item } }));
                            });
                            setMeals(prev => prev.map(x => x.id === item.id ? { ...x, cookCount: x.cookCount + 1 } : x));
                            setBankPlanningId(null);
                          }}
                          onClose={() => setBankPlanningId(null)}
                        />
                      </div>
                    )}
                  </div>
